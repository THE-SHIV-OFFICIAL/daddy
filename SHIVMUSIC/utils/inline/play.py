...
def playback_row(chat_id, playing=True):
    """Compact tablet-style player row shared by the main bot."""
    s_map = get_style_map()
    return [
        create_btn(
            text="⏮",
...
def stream_markup2(_, chat_id, playing=True):
    return stream_markup(_, chat_id, playing=playing)


def stream_markup_timer2(_, chat_id, played, dur, playing=True):
    return stream_markup_timer(_, chat_id, played, dur, playing=playing)
